...
describe('VideoCallService', () => {
  ...
  describe('generateVideoCallLink', () => {
    it('should generate a valid video call link', () => {
      ...
      expect(link).toContain('config.enableLobby=true');
    });
  ...
  describe('isValidVideoCallUrl', () => {
    it('should validate links generated by the service', () => {
      const validUrl = service.generateVideoCallLink(
        'appt-12345678',
        'Salon Test',
      );
    ...
    it('should reject non app-specific room name formats', () => {
      const invalidUrl =
        'https://meet.jit.si/complex-room-name-with-many-parts';
      expect(service.isValidVideoCallUrl(invalidUrl)).toBe(false);
    });

    it('should validate link against expected appointment when provided', () => {
      const validUrl = service.generateVideoCallLink(
        'appt-12345678',
        'Salon Test',
      );
      expect(service.isValidVideoCallUrl(validUrl, 'appt-12345678')).toBe(true);
      expect(service.isValidVideoCallUrl(validUrl, 'appt-99999999')).toBe(
        false,
      );
    });
  ...
  describe('extractRoomNameFromUrl', () => {
    it('should extract room name from generated Jitsi URL', () => {
      const url = service.generateVideoCallLink('appt-12345678', 'Salon Test');
  ...
  describe('Integration and edge cases', () => {
    it('should generate and validate consistent video call links', () => {
      ...
      expect(service.isValidVideoCallUrl(link, appointmentId)).toBe(true);
    });
    ...
    it('should generate different links for different appointments', () => {
      const firstAppointmentId = 'appt-00000001';
      const secondAppointmentId = 'appt-00000002';
      const link1 = service.generateVideoCallLink(firstAppointmentId, 'Salon');
      const link2 = service.generateVideoCallLink(secondAppointmentId, 'Salon');

      expect(link1).not.toBe(link2);
      expect(service.isValidVideoCallUrl(link1, firstAppointmentId)).toBe(true);
      expect(service.isValidVideoCallUrl(link2, secondAppointmentId)).toBe(
        true,
      );
    });
    ...
    it('should generate unique custom links with same parameters', () => {
      ...
      expect(service.isValidVideoCallUrl(link1, 'appt-00000001')).toBe(true);
      expect(service.isValidVideoCallUrl(link2, 'appt-00000001')).toBe(true);
    });

    it('should correctly handle case sensitivity in URL validation', () => {
      const url = 'https://MEET.JIT.SI/salon-rdv-12345678-abcd1234';
